...
export default function PostCard({ post, onReport }: PostCardProps) {
  ...
  const handleShare = async () => {
    const shareData = {
      title: `${post.plant_name} at ${post.nursery_name}`,
      text: `Check out these ${post.plant_name} seedlings available at ${post.nursery_name} on WeFarm!`,
      url: `https://wefarm.live/nursery/${post.seller_id}`,
    };
  ...
  return (
    <article className="post-card group animate-fade-in" id={`post-${post.id}`}>
      ...
      <div className="flex items-center gap-3 px-4 pb-4">
        ...
        <button
          onClick={handleShare}
          className="w-12 flex-shrink-0 flex items-center justify-center py-3 bg-surface-100 hover:bg-surface-200 rounded-xl text-surface-600 hover:text-surface-900 transition-all duration-200 active:scale-[0.97]"
